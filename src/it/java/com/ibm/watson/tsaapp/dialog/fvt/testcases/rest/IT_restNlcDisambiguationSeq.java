...
public class IT_restNlcDisambiguationSeq extends SetupMethod{
	...
	public IT_restNlcDisambiguationSeq(BaseQuestion quest) {
		this.question = quest;
	}

	@Parameters(name = "NLC disambiguation questions")
	public static Iterable<BaseQuestion> data() {

		ArrayList<BaseQuestion> questions = new ArrayList<BaseQuestion>();		
		questions = Utils.getQuestions(NLC_DISAMBIGUATE_QUESTION, true);
	...
	/**
	 *<ul>
	 *<li><B>Info: </B>Ask a series of questions of each type which will invoke a disambiguation response from WDS</li>
	...
	public void maintainConversationId() {
        ...
        logger.info("Asking the disambiguation-type question.");
        logger.info("INFO: Question - " + question.getText());
        question.ask(api);
