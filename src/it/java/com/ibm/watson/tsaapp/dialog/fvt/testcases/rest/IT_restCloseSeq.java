...
public class IT_restCloseSeq extends SetupMethod{
	...
	public IT_restCloseSeq(BaseQuestion quest) {		
		this.question = quest;
	}

	@Parameters(name = "Closing sequence question")
	public static Iterable<BaseQuestion> data() {

		ArrayList<BaseQuestion> questions = new ArrayList<BaseQuestion>();		
		questions = Utils.getQuestions(CLOSING_QUESTION, true);
	 ...
	 *<li><B>Step: </B>Ask closing question using getResponse rest api call</li>
	 *<li><B>Verify: </B>Validate that the response ask the user to clarify if the user uses a closing sequence that they get an expected response</li>
	 *</ul>
	 */
	@Test
