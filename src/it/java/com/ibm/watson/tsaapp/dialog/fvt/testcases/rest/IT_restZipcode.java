...
package com.ibm.watson.tsaapp.dialog.fvt.testcases.rest;
...
import com.ibm.watson.tsaapp.dialog.fvt.config.API_TestWatcherRule;
import com.ibm.watson.tsaapp.dialog.fvt.config.RestAssuredManager;
import com.ibm.watson.tsaapp.dialog.fvt.config.SetupMethod;
import com.ibm.watson.tsaapp.dialog.fvt.config.Utils;
import com.ibm.watson.tsaapp.dialog.fvt.rest.RestAPI;


@RunWith(value = Parameterized.class)
public class IT_restZipcode extends SetupMethod{
	...
	public IT_restZipcode(BaseQuestion quest) {
		this.question = quest;
	}

	@Parameters(name = "Zipcode question")
	public static Iterable<BaseQuestion> data() {	

		ArrayList<BaseQuestion> questions = new ArrayList<BaseQuestion>();		
		questions = Utils.getQuestions(ZIP_JSON_FILE, true);
	...
	/**
	 *<ul>
	 *<li><B>Info: </B>Test generic Zipcode questions answer with expected response</li>
	 *<li><B>Step: </B>Connect to Showcase Dialog app using initchat rest api call</li>
	 *<li><B>Step: </B>Ask a question that requests the user to add the zip code</li>
	 *<li><B>Verify: </B>Validate that the response is a probing question (i.e. What's your ZIP code?)</li>
	 *</ul>
	 */
	@Test
